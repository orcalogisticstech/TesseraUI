...
export function CopilotPanel() {
  ...
  const panelVisible = copilotOpen;
  const desktopShellWidth = copilotOpen ? copilotWidth : 0;
  ...
  return (
    <aside
      className={`fixed right-0 top-0 z-40 h-screen w-full md:w-[400px] lg:sticky lg:z-20 lg:w-[var(--tessera-copilot-shell-width)] lg:overflow-hidden ${isResizing ? "lg:transition-none" : "lg:transition-[width] lg:duration-[250ms] lg:ease-out"} ${panelVisible ? "pointer-events-auto" : "pointer-events-none"} lg:pointer-events-auto`}
      style={panelStyle}
    >
      <div
        className={`absolute inset-y-0 right-0 w-full overscroll-contain border-l md:w-[400px] lg:w-[var(--tessera-copilot-width)] ${isResizing ? "transition-none" : "transition-transform duration-[250ms] ease-out"} ${panelVisible ? "translate-x-0" : "translate-x-full"}`}
        style={{
